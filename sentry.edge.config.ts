// Sentry — Edge runtime (middleware + any edge routes). Errors only: tracing
// and replay are OFF (quota + privacy). Enabled ONLY on Vercel
// production/preview so local dev never spends the error quota. All
// credentials come from env — never hardcode.
import * as Sentry from '@sentry/nextjs';
import type { ErrorEvent } from '@sentry/nextjs';

// Scrubber duplicated verbatim from sentry.server.config.ts — the edge bundle
// is a separate entry point and must stay import-free. See that file for the
// rationale (application answers, SMS bodies, private notes, payment fields).

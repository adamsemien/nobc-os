...
| Field | Value |
|---|---|
| **State** | 🟢 MERGED + IN PRODUCTION — `feat/event-builder-rebuild` (M1–M4-PAY + builder rebuild + D6 + Night + Loose Ends + Ways-In Phase A) merged to `main` via `ecf1d07` 2026-07-03; deployed to production (Adam verified via Vercel, 2026-07-10). Person-primary retrofit drafted on `feature/gate-engine-person-primary-retrofit` (schema + SQL artifact, NOT applied). Builder tighten-and-fix pass (edit-route cutover to the new builder + display honesty) committed on local branch `event-builder-fix`, unmerged. Guest-preview scaling fix (BuilderShell iframe renders at a fixed 1280px viewport, scaled to fit the panel) committed on branch `san-antonio`, gated for Adam's review, unmerged |
| **V1 item** | None — post-July-11 platform work (v3 roadmap: engine, guest render, Builder, cutover surfaces + pay all built) |
| **Last updated** | 2026-07-14 |
| **Owner** | Adam |
| **Blocked on** | (0) Adam reviews the gated `san-antonio` commit (builder guest-preview desktop-scaling fix, one code file + this doc) — do not merge until he approves. (1) Adam reviews + merges `event-builder-fix` (edit flow → new builder, honest gateless-legacy Access panel, hero-upload two-tap fix, tiers bridge link). (2) Adam reviews `feature/gate-engine-person-primary-retrofit` and runs `prisma/sql/gate-engine-person-primary-retrofit.sql` (sections 1→2→3) manually in Neon — production is his act. (3) DB-application status of the earlier gate SQL artifacts (`gate-engine-m1.sql`, `ways-in-phase-a.sql`, `event-builder-phase-a/f.sql`, `stripe-event.sql`) against PRODUCTION is undocumented here — the code referencing these tables is live in prod, so they are presumably applied, but Adam should confirm and record it (recon Q3). (4) `application_submitted` emitter name reconciliation with the CRM owner (unchanged). |
| **Next** | 2026-07-14 — GUEST-PREVIEW SCALING FIX committed on branch `san-antonio` (gated for Adam's review, NOT merged/pushed): the builder's "Guest preview - exactly as they will see it" iframe was rendering the guest page's MOBILE layout (fixed sticky CTA bar overlapping the description tail, "A few steps unlock your access" helper hidden) because media queries inside an iframe evaluate against the iframe's own width — and the panel is only half the operator's screen, under the guest page's `lg` 1024px breakpoint. Fix is local to `BuilderShell.tsx` only: the iframe now renders at a fixed 1280px viewport width and is scaled down (`transform: scale`, ResizeObserver-driven) to fit the panel, so the preview always shows the desktop render — pixel-identical to the standalone `/e/[slug]` page. RsvpCard + templates + public loader untouched (structurally cannot affect real guests). Proof: tsc clean, next build clean (157/157 pages), scaling mechanism verified in Chromium at 1600/1100/500px operator widths (desktop card + helper text visible, no sticky bar, no scrollbars/clipping at any width). NEXT ACTION: Adam reviews + merges the gated `san-antonio` commit. Prior context: 2026-07-11 — BUILDER TIGHTEN-AND-FIX PASS committed on `event-builder-fix` (local worktree branch, NOT pushed/merged; Adam's post-GO order, decisions locked): (1) edit flow now routes to the WYSIWYG builder — events-table row links + the event dashboard's Edit button target `/operator/events/[id]/builder`; the old tabbed dashboard stays reachable (BuilderShell footer link) for attendees/applications/check-in. (2) Ticket-tiers bridge: honest link from the builder's Access section to the legacy settings tiers section (`?tab=settings#tiers`, new anchor) until tiers are ported into the Gate Engine — tiered events are NOT blocked or conditionally routed. (3) Display honesty for gateless legacy events: `getBuilderState` returns a `legacyDoor` summary (READ-ONLY `parseEventAccess`/`resolveAccessForViewer` calls, resolved for the same anon viewer the preview renders); the rail shows "No gate yet - this event's door is governed by legacy access: …" instead of falsely claiming Open. NO auto-mint of Gate rows from legacy eventAccess/tiers — explicitly out of scope per Adam's order; that migration needs its own authorization. (4) HeroImageUpload two-tap defect fixed (hover-only overlay removed; one always-visible control per action, Remove is its own corner icon button). (5) PAY chip price input saves on blur like the rest of the rail. Proof: tsc clean, next build clean, unit suite 1202 pass / 4 fail (the 4 = the known pre-existing apply-create-route failures, untouched files). NEXT ACTION: Adam reviews + merges `event-builder-fix`, then decides the tiers→gate port. Prior context: 2026-07-10 — DOC RECONCILIATION + PERSON-PRIMARY RETROFIT drafted (branch `feature/gate-engine-person-primary-retrofit`, local only): nullable `personId` dual pointers on GateProof + GateSession, `fromPersonId`/`toPersonId` on GrowthEdge; member FKs keep RESTRICT (pinned explicitly in schema.prisma); the three NOT-NULL widenings are isolated in the SQL's Section 2 (Adam-approved exception to the refuse-ALTERs rule); 4 out-of-band partial uniques in Section 3 (CLAUDE.md landmine entries once applied). Proof: `prisma generate` clean; `tsc --noEmit` clean with ZERO code changes (no engine/verifier/bridge edits needed); 161 pure gate-engine + ways-in unit tests green (DB suites deliberately not run — zero DB contact this session). Ground-truth recon that triggered all this: `docs/gate-engine-m1-recon.md`. NEXT ACTION: Adam reviews the branch, runs the retrofit SQL in the Neon console, and records prod DB-application status of the prior gate SQL artifacts (recon Q3). Spec disposition is settled (recon Q1): v3 substrate stays, flat spec's schema retired, flat POLICY decisions carried over (already match live code). The 2026-07-01→07-06 build log that previously lived in this row is preserved in this file's git history. |
...
app/operator/events/[id]/builder/             ← (Rebuild) WYSIWYG builder - true anon preview + control rail;
                                                 honest gateless-legacy panel + tiers bridge (event-builder-fix);
                                                 preview iframe fixed 1280px viewport + scale-to-fit (san-antonio)
lib/builder/                                  ← (Rebuild) typed builder action layer (UI + AI composer share it);
                                                 legacyDoor summary via read-only lib/event-access calls

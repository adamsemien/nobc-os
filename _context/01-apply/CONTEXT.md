...
| **Owner** | Adam |
| **Blocked on** | Nothing |
| **Next** | **2026-06-09 UX audit (`_context/_audit/UX-FRICTION-AUDIT.md`, PR #65):** P0 — apply form state is client-only, a refresh loses the whole ~25–40 min application; the resume banner is wired but never populated — wire it to the existing draft `PATCH /api/apply/membership/[id]` (audit #1). P1 — photo-upload failures are silently swallowed (`MembershipForm.tsx` ~L787, audit #9). P0 — pending applicants have no status surface after `/apply/thanks` (audit #4). Copy fixes C11–C15 are proposals only — apply-flow copy is locked, Adam signs off. — Backlog: reconcile the three competing answer-key generations (see Known issues). No structural form changes. (2026-05-21: demo seed rewritten to emit the live form's real dotted keys; one-time archetypeScores 0–1 → 0–100 migration done.) |

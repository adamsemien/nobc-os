...
| **Data Integrity Build A (committed on `data-integrity`, unmerged)** | 2026-07-12 — the apply flow's ONLY touch: the submit route now stamps the new `Application.submittedAt` completion signal — one line in the existing scoring update, plus the BLOCKED-watchlist auto-reject branch (the applicant did click Submit there too, matching the prod backfill rule `status <> PENDING`). PURPLE needs nothing (falls through to the scoring update). Frozen zones (scoring.ts, archetypes, MembershipForm question/scoring logic, handleSubmit) untouched. Also repaired on this branch: `tests/unit/apply-submit-route.test.ts` finally COLLECTS and runs green 9/9 (the long-standing "load error" was tsconfig `jsx: preserve` leaking into vitest's oxc transform — fixed in `vitest.config.ts` — plus the db mock predating the 2026-07-02 `$transaction` advisory-lock refactor and the Reveal-B `scoreApplication` shape). The 4 `apply-create-route.test.ts` failures (Clerk server-only) remain the known baseline. Stage 02's CONTEXT.md carries the full Build A record (operator queue, approve-guard, SQL blocks Adam runs). |

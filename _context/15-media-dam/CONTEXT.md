...
| **Next** | Open a PR for `feat/canto-migration` once the Canto migration finishes writing prod. Grid Round 2 is complete on this branch (drag-rearrange, infinite scroll, elevated desktop multiselect, full mobile/touch layer; Draft+Form motion + SR-announcement specs applied, no TODO markers remaining). **Device QA still owed:** touch gestures (swipe feel, pinch accuracy, `navigator.vibrate`) were verified by code review only — exercise on a real iPhone (or Chrome touch emulation) before merge. Then preview-verify vibe search + facets + grid drag/reorder against the real 1,838-asset library with `REPLICATE_API_TOKEN` set. Phase 5 (MCP tools) or Phase 6 (`<DamPicker>`) next after merge. |

...
| **Next** | Media-page UI cleanup PR (`fix/media-page-ui-cleanup`) open — preview-verify all six items on the deployed preview, then merge. After that, pick up Phase 3 (Timeline / Moment Map; spec in `docs/superpowers/specs/2026-05-27-dam-phase-3-timeline-design.md`) or jump to Phase 5 (MCP tools at `/api/mcp`) or Phase 6 (`<DamPicker>` modal + event-editor hero integration). |

...
| **Next** | Open a PR for `feat/canto-migration` once the Canto migration finishes writing prod and the branch is reviewed; preview-verify vibe search + facets + lightbox against the real 1,838-asset library on a deploy with `REPLICATE_API_TOKEN` set. Then pick up Phase 5 (MCP tools at `/api/mcp`) or Phase 6 (`<DamPicker>` modal + event-editor hero integration), or Phase 3 (Timeline) now that real photography exists. Two known follow-ups: client-side color filter can show a short page under infinite scroll (acceptable per spec); consider persisting a color-bucket column if server-side color filtering is ever needed. |

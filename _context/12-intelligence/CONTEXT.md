...
| **Owner** | Adam |
| **Blocked on** | Sponsor pilot definition. The Sponsor Intelligence **page** (`/operator/intelligence/sponsor`) and its "Sponsors" nav entry now authorize by **Clerk org membership** (`auth()` + `getMemberWorkspaceId`), not `requireRolePage(ADMIN)` — any workspace org member may view, matching the House Phone model. Its "Sponsors" **nav** entry keeps `adminOnly` and now resolves correctly via the Clerk-org-aware role floor (`getEffectiveRole`, 2026-05-26). The `/api/intelligence/{compose,reports}` POST routes are now `requireRole(STAFF)` (2026-05-26); GET reads stay open to any resolved member. |

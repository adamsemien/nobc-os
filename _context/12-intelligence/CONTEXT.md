...
| **Owner** | Adam |
| **Blocked on** | Sponsor pilot definition |
| **Next** | Build sponsor-facing dashboard per NoBC Intelligence pilot spec |
...
app/operator/intelligence/_components/IntelligenceView.tsx          ← top-level layout (tabs + tiles + drill)
...
5. **AI narration is bounded.** `insight-generator.ts` uses claude-sonnet-4-20250514, no model substitution. Narratives are short (1–2 sentences) and never invent numbers — only describe what the metric returned.

...
| **Rich email editor slice (Phase 1 toolbar + inline images on branch `email-image-upload-dam-insert`/`las-vegas`, awaiting merge/deploy; all earlier fixes merged to main)** | **2026-07-14 (latest, branch `las-vegas`): Phase 1 - real editing UI + inline images on event.reminder, awaiting Adam's merge + live verify.** Recon finding: the formatting layer was ALREADY live - `EmailEditor` unconditionally renders the package's bubble menu (bold/italic/underline/strike/code/uppercase + node selector + link + alignment) and slash-command menu, and the package StarterKit registers all formatting marks (Cmd+B bolded before any toolbar existed; pinned by 2 new tests in `email-editor-schema.test.ts`). Phase 1 added: (1) **inline images** via the package's email-safe image node, activated by `EmailEditor`'s `onUploadImage` prop (the package appends it to the extension list even with a custom `extensions` prop - NOT the crash-class trap; serializes to react-email `<Img>` with width as an HTML **attribute**, the Outlook-safe form; paste + drag-drop handled by the package's ProseMirror plugin). (2) **Upload chain reuses the DAM end-to-end** (zero new routes): `POST /api/media/dam/upload` pinned to a resolve-or-create **"Email Images"** folder (type BRAND; promise-cached client-side; dev-DB id `cmrlc15000000cmiteig9ri5g`, created 2026-07-14; prod auto-creates on first live upload) → `GET /api/media/dam/asset/[id]/public-link` → stable `/i/{token}` HMAC URL (NO expiry - the only URL form safe in an email; signed R2 URLs expire in the inbox). Mint fails closed 503 if `DAM_PUBLIC_LINK_SECRET` unset; secret confirmed in Vercel Preview + Production and reads at runtime (verified locally with the real preview value). CAVEAT: `R2_*` and prod `DATABASE_URL`/`DAM_PUBLIC_LINK_SECRET` are Vercel **sensitive** (unpullable) vars → the full upload E2E is only verifiable on a deployed env. (3) **Sizing:** picker path sets `width = min(600, naturalWidth)` at INSERT time in the same transaction that creates the node (src `?w=1200` for retina); paste/drop path (package owns that insert transaction and its uploadImage returns only `{url}`) falls back to serving `?w=600` flat, so pasted images can never exceed the 600px body. (4) **"Insert image" button + hint strip** ("Select text to format - type / for headings, lists and more") above the editor; bubble menu themed via `--re-*` CSS vars on the editor wrapper from `EMAIL_THEME` constants only (the slash menu portals to `document.body` and keeps the package default theme - accepted). tsc 0, `next build` clean (warning lines byte-identical to clean-main baseline via stash-compare), unit suite green minus the 4 pre-existing `apply-create-route` failures. Files: `NoBCEmailEditor.tsx` + `tests/unit/email-editor-schema.test.ts` only. Live checks post-deploy: bubble menu on select-text, button image insert lands inline, paste-image works, preview===send with the image, Gmail render, "Email Images" folder auto-creates in prod. - **Prior entry (branch `basseterre`, since merged):** preview-overlap + em-dash fixes. Preview overlap root cause was NOT the recon's Tailwind-preflight/padding theory — it was `NOBC_EDITOR_THEME`'s `lineHeight: 1.75` (bare number): the package's `transformToCssJs` appends the panel's `%` unit to bare numbers, producing `line-height:1.75%` (~0.3px) in BOTH the injected editor CSS and every composed `<p>` inline style, collapsing all lines. Paddings survive fine; the preview iframe is CSS-isolated so preflight never touched it. One-line fix: `lineHeight: '175%'` (NoBCEmailEditor.tsx). NOT preview-only: one rich-editor Save would have persisted the broken line-height into sent bodyHtml (the confirmed-good test send read the still-default DB row). Em-dash law now enforced at both chokepoints: PATCH `/api/operator/settings/communications` sanitizes subject/bodyHtml/bodyText via `toBrandPunctuation` before update, and `sendTemplatedEmail` (lib/email.ts, authorized narrow unfreeze) wraps the three `interpolate()` outputs — also catches dashes arriving via variables; regex is HTML-safe (only literal U+2013/U+2014, never markup; known accepted gaps: raw dash inside a URL, entity-encoded `&mdash;`). `editorConfig` deliberately unsanitized (both chokepoints re-sanitize downstream). tsc 0, `next build` exit 0 (error/warning counts identical to clean-main baseline), unit suite 1299 passed with only the 4 pre-existing `apply-create-route` failures. **2026-07-14 (later): the slice merged to main and crashed prod** — every event.reminder editor mount threw `RangeError: Schema is missing its top node type ('doc')`; main was reverted clean (`a0cd007`). Root cause (verified in `@react-email/editor@1.6.9` source): `EmailEditor`'s `extensions` prop REPLACES the package defaults (`extensionsProp ?? [StarterKit, Placeholder, EmailTheming]`) — `NoBCEmailEditor` passed `extensions={[MergeVariable]}`, dropping StarterKit's Document node (and silently disabling the `theme`/`placeholder` props, which only apply in the default branch — the NoBC theming never would have rendered either). Fix on `fix-tiptap-doc-node-crash`: revert-of-revert (`bbecc78`) + fix commit (`e2aa76d`) — exported `EDITOR_EXTENSIONS = [StarterKit.configure(), Placeholder.configure(...), EmailTheming.configure({ theme: NOBC_EDITOR_THEME }), MergeVariable]`, new direct dep `@tiptap/extension-placeholder@^3.27.4` (dedupes with the editor's own copy), regression test `tests/unit/email-editor-schema.test.ts` (broken shape throws / fixed schema has doc+mergeVariable / theme carries Georgia+red+cream), vitest.config gained inline empty PostCSS config. tsc 0, `next build` exit 0 (274 routes). NOTE: the `editorConfig` column was already applied in Neon before the crash and survived the revert — the ALTER TABLE step below is DONE. Original slice scope: React Email 6 adoption + `@react-email/editor` rich authoring for Settings → Communications, `event.reminder` ONLY (the other 9 templates stay on the string editor). Phase 0: `@react-email/components` removed, `react-email@6.8.1` + `@react-email/editor@1.6.9` added (`@react-email/ui` skipped per Adam), 5-template byte-diff smoke gate passed after an approved `borderTop` fix (RE6's style-merge reorder had flipped Hr divider colors to #eaeaea; harness: `scripts/smoke-render-emails.tsx`; remaining deltas - title tag, body dir/lang, preheader chars, Container padding relocation, mso px unit, preload-hint removal - signed off as invisible). Phase 1: `EmailTemplate.editorConfig Json?` (schema edited + client generated; **SQL NOT applied - Adam must run `ALTER TABLE "EmailTemplate" ADD COLUMN "editorConfig" JSONB;` in Neon BEFORE deploying this branch**, the generated client selects the column), `emails/theme.ts` (sanctioned literal-hex email exception, Georgia per Adam - no Cormorant), `lib/email-interpolate.ts` (`flatten`/`interpolate` extracted verbatim from `lib/email.ts` - the ONE interpolator, shared by send path + operator preview), `lib/email-sample-data.ts` (shared SAMPLE_EMAIL_DATA), `emails/editor/merge-variable.tsx` (inline EmailNode: sample value shown in-editor, literal `{{key}}` serialized into sent HTML), `NoBCEmailEditor.tsx` (dynamic ssr:false wrapper, NoBC ThemeConfig), sandboxed-iframe preview = send, `POST /api/operator/settings/communications/send-test` (settings.edit-gated, recipient resolved from the Clerk session only, no identities arg → log rows carry null memberId/personId → can never emit MemberEngagementEvent), PATCH `bodyHtml` cap 60k → 200k + optional `editorConfig`. New deps: `happy-dom` (required transitively - `@tiptap/html`'s server entry must resolve at build). Verified: tsc 0, `next build` exit 0 (error/warning counts identical to pre-existing baseline), unit suite green minus the 4 pre-existing `apply-create-route` failures Adam logged separately. |
| **The Line (committed on `feat/the-line-egg`, unmerged)** | 2026-07-12 — second Back Room arcade game, sibling of Last Call: `TheLineGame.tsx` (canvas + one rAF loop, 3-lane runner toward THE DOOR — dodge fiction obstacles, choose velvet-rope gates that do crew math (+2 / ×2 / −half — the access-gate wink), throw limited "good vibes" to clear lanes; 3 rounds, 3 lives, rank ladder to HEAD OF DOOR; best in `localStorage['nobc-theline-best']`). Canvas colors resolved from design tokens via `getComputedStyle` at mount (stricter than the Frogger hex precedent) so it inherits the room's Darkroom palette; audio reuses the room's procedural `pluck`/`buzz`; reduced-motion drops shake/particles, game stays playable. Mount: `BackRoomEasterEgg` gained phase `'theline'` + a flickering neon sign under the matchbook — "The line's around the block. Run it?" — Esc backs out to the room. Deliberately NOT in the DevToolbar egg list ("keep it between members"). Client-only, zero network, zero member data, zero new deps. `tsc` 0, `next build` green. |
| **Owner** | Adam |
| **Blocked on** | Email-editor slice: Adam merging branch `email-image-upload-dam-insert` (Phase 1 toolbar + inline images), deploying, then the live checks: (a) bubble menu appears on select-text, (b) Insert image uploads + lands inline, (c) preview matches send with the image, (d) a real test send renders the image in Gmail - the emailed `<img>` src is the stable `/i/{token}` public link, which needs `DAM_PUBLIC_LINK_SECRET` (already set in Preview + Production). (Earlier `basseterre` overlap + em-dash fixes and the crash fix are merged; the `editorConfig` Neon SQL is applied - no DB step remains.) Otherwise nothing hard. RBAC gates every `/api/operator/*` write (STAFF; refunds/settings/team ADMIN), the operator RSVP-bypass branch in `/api/m/.../access/*` (STAFF), `/api/stripe/refund` (ADMIN), and now `/api/agent/*` + `/api/intelligence/{compose,reports}` POST (STAFF, 2026-05-26). A Clerk-org floor (`getEffectiveRole`) means a Clerk org admin with no `WorkspaceMember` row is treated as ADMIN (never locked out). Remaining (deferred): only the `/api/mcp` write path (Bearer-authed — needs its own design). `/api/check-in/*` is intentionally excluded (`CHECKIN_SECRET` bearer, not a Clerk session). |
| **Next** | **2026-07-14 (Phase 1 toolbar + inline images, branch `email-image-upload-dam-insert`):** Adam merges + deploys, then live-verifies on prod Communications: bubble menu on select-text, Insert image → inline in body, preview===send, real test send renders the image in Gmail. Then Phase 2: DAM picker (reuse this phase's insert plumbing + the existing DAM folder-tree UI) and decide which of the other 9 templates joins the rich editor. ~~2026-07-14 (overlap + em-dash fixes, `basseterre`)~~ - superseded: merged to main. ~~2026-07-14 (email-editor crash fix): preview-gate `fix-tiptap-doc-node-crash`~~ — superseded: that branch merged to main (`9f63ef6`). ~~Previous 2026-07-14 plan (review `event-reminder-email-slice`, run ALTER TABLE, then merge)~~ — superseded: that branch merged, crashed prod (missing `doc` top node), was reverted (`a0cd007`), and is rebuilt fixed on `fix-tiptap-doc-node-crash`. **2026-07-03 (pre-cutover cleanup):** sidebar Settings pinned-to-bottom restored - the June-22 `html, body { overflow-x: hidden }` mobile rule had made body a scroll container and silently broken `position: sticky` in the rail (Settings floated mid-rail on long pages); fixed to `overflow-x: clip` in `globals.css` (clips without creating a scroll container; mobile no-sideways-scroll verified at 390px). Same commit: site-wide OG share cards + NoBC favicon.ico (was the Create-Next-App Vercel default) - `og-default.png`/`og-apply.png` are FUNCTIONAL PLACEHOLDERS pending final art from Chloe. **2026-06-11 (branch `back-room`, pushed for review):** review/merge The Back Room easter egg + Darkroom theme + Last Call door game — type "knockknock" (desktop) or knock tap-tap·pause·tap-tap on the page (touch) for the speakeasy cinematic; the matchbook on the card opens Last Call (swipe-to-decide door game, 18 procedural guests, localStorage high score only); "Kill the lights" swaps to Darkroom (also in Cmd+K + Appearance with an Egg badge). Client-only; no network, no data access, all guests procedurally faked. Then: only the `/api/mcp` write path remains ungated (Bearer-authed external clients — design a dedicated check before applying a gate; `/api/agent/*` and `/api/intelligence/{compose,reports}` were gated STAFF on 2026-05-26). (Add Member create is now `requireRole(STAFF)` and its button is hidden for READ_ONLY via `MembersView canAddMembers`; the GUEST-filtered-from-directory note still stands — `GET /api/operator/members` excludes `status: GUEST`.) Optionally trim the now-unused birthdays/throwbacks fetch from `page.tsx`. (Audit 2026-05-21: the only "Open The Room" entry on this dashboard is gated to today-only events (`page.tsx:170`) — see Audit findings below.) **2026-05-27, PR #34:** all 8 operator data pages standardized to full width — Events, Applications, Members, Activity (the four list/table pages) + Event detail, Application detail, Member detail, New Event (the four detail/form pages) — dropped their `mx-auto max-w-[…]` content caps (→ `w-full`) and adopted the dashboard's `px-6 sm:px-10 lg:px-14 xl:px-20` gutters. **Settings/forms pages intentionally keep their narrow caps — these are by design, not bugs.** (Events/Applications pages render here but are owned by stages 03/02 — convention recorded here as the operator-shell owner.) **2026-05-27, PR #33:** Settings now has a **Developer** section that links to the DevToolbar (`OpenDevToolbarButton`, dev-gated by `DEV_USER_IDS`) — see Stage 13. **2026-05-28, nav simplification:** sidebar trimmed 12 → 7 (Dashboard, Events, Applications, Members, House Phone, Media, Intelligence) + Settings footer. Removed from rail: Check-in (per-event entry via `EventActionBar` is the door), Team + Lists (already surfaced as Settings cards), Sponsors (now a tab inside Intelligence — Community / Sponsors — Sponsors tab hidden for non-ADMIN), Activity (already rendered as the dashboard's `folio 03` "Recent activity" widget; link relabeled "Full log →" → "View all →"). Preview Site + Apply Form + `UserButton` moved out of the sidebar into a non-sticky inline utility strip at the top-right of `<main>` in `layout.tsx`. Compact-sidebar wordmark fixed `NBC` → `NoBC` (root CLAUDE.md naming rule). `OperatorNav`'s `isAdmin` prop is now unused (no more `adminOnly` items) but kept on the signature — `IntelligenceTabs` owns the Sponsors-tab hide via its own `isAdmin` prop resolved server-side from `lib/operator-role.ts:isAdmin`. |

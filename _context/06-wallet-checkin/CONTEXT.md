...
| **Next** | **2026-06-16: public no-login ticket page + email fallback repoint + dead wallet CTA hidden (branch `feat/public-ticket-page`, NOT merged).** New PUBLIC `/ticket/[rsvpId]` (`app/ticket/[rsvpId]/page.tsx` + resolver `lib/ticket-view.ts`) so guest ticket-holders always reach their QR with no login: public by omission (not in the Clerk matcher), keyed by the unguessable rsvpId capability token like `/api/qr`, reads event + member through the relation, fail-closed `notFound()`, never exposes `memberQrCode` (QR delivered only via `<img src="/api/qr/[rsvpId]">`). `rsvpConfirmedEmail`/`compTicketEmail` fallback links repointed off the Clerk-gated `/m/.../confirmed` + `/check-in/verify` to `/ticket/[rsvpId]` (this is Stage 05's flagged WS1c). Dead Apple/Google wallet CTA on `/m/events/[slug]/confirmed` gated behind a `walletConfigured` env check (hidden until `PASSNINJA_API_KEY` + `PASSNINJA_ACCOUNT_ID` are set; `TODO(wallet)`). tonone warden (SHIP) + proof reviews folded in. tsc + next build + 517 unit tests green. Next action: Adam review + merge `feat/public-ticket-page`. Earlier: **2026-06-16: `/api/qr/[id]` hardened (branch `chore/overnight-harden-2026-06-15`, NOT merged).** The public QR image endpoint now wraps its RSVP lookup + PNG encode in try/catch: a DB or encoding error logs with the id and returns 404 (fail closed) instead of 500. The 1-day cache value and the rsvpId to member to memberQrCode indirection are unchanged. New `tests/unit/qr-route.test.ts` (7 tests) locks the 200 PNG, the door-credential indirection, and 404 on unknown/malformed/QR-less/DB-error. A separate read-only audit, `_context/_audit/CHECKIN-RBAC-SCOPING-2026-06-15.md`, scopes the next workstream (scanner behavior gaps + a proposed door-staff RBAC tier). Earlier: Set `CHECKIN_SECRET` in Vercel (server signing key for the new check-in tokens) and REMOVE the now-unused `NEXT_PUBLIC_CHECKIN_SECRET`. Set `PASSNINJA_*` to light up native wallet passes. Optional: one-time backfill `memberQrCode` for Members created before 2026-05-21. |
...
app/ticket/[rsvpId]/page.tsx                    ← PUBLIC no-login ticket page (safety net for the email fallback link; rsvpId resolves to event + QR via <img src="/api/qr/[rsvpId]">; fail-closed notFound; never exposes memberQrCode). Added 2026-06-16 (feat/public-ticket-page)
lib/ticket-view.ts                              ← resolver for /ticket (rsvpId capability token, mirrors /api/qr; reads event + member through the relation; never selects memberQrCode). 2026-06-16
tests/unit/ticket-view.test.ts                  ← /ticket coverage: behavioral resolver (db-mocked) + page source-scan. 2026-06-16
app/m/events/[slug]/confirmed/page.tsx          ← wallet CTA gated behind walletConfigured env (PASSNINJA_API_KEY + PASSNINJA_ACCOUNT_ID); dead 503 button hidden until passes go live (TODO(wallet); 2026-06-16)
```

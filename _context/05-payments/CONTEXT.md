...
| Field | Value |
|---|---|
| **State** | ✅ Shipped (🟡 buyer-pays-fee on branch `feat/pass-stripe-fee`, not yet merged; earlier checkout-hardening branch status unchanged) |
| **V1 item** | #9, #10, #21 (comp tickets — payment-side bookkeeping), #25 (compliance pages) |
| **Last updated** | 2026-07-02 |
| **Owner** | Adam |
| **Blocked on** | Adam's review + merge + deploy of `feat/pass-stripe-fee` before July 11 (No Bad Saturday). Also standing: compliance pages legal review. |
| **Next** | **2026-07-02: buyer-pays-Stripe-fee BUILT (branch `feat/pass-stripe-fee`, worktree `pass-stripe-fee`, NOT merged).** Both live checkout intent routes (`/api/e/[slug]/access/payment-intent` + `/api/m/events/[slug]/access/payment-intent`) now gross the ticket price up so the buyer covers the 2.9% + 30c Stripe fee (`lib/ticketing/buyer-fee.ts`: charged = round((price + 30) / 0.971); $25.00 charges $26.06, NoBC nets $25.00). PI metadata gains `feeCents`; RSVP.amountCents + the JSON response carry the charged total (refund math unchanged); idempotency keys carry the charged amount. Pay step (`PayForm` in `EventAccessFlow.tsx`, display-only lines) now shows Luma-style line items - Ticket / Service fee / Total - derived exactly from the one amount the flow already passes (`splitChargedCents` is a proven-exact inverse; 300k-price sweep in `tests/unit/buyer-fee.test.ts` + route acceptance in `tests/unit/payment-intent-buyer-fee.test.ts`). Always-on by design; the per-event toggle is the fast-follow via the dormant `Event.serviceFee*` columns (deliberately not wired - July-11 tactical scope). Next action: Adam reviews line-item copy + merges + deploys before July 11. Earlier: **2026-06-16: WS1c BUILT (branch `feat/public-ticket-page`, NOT merged).** `rsvpConfirmedEmail`/`compTicketEmail` fallback links repointed off the Clerk-gated `/m/.../confirmed` + `/check-in/verify` to the new PUBLIC `/ticket/[rsvpId]` page so guest buyers always reach their QR with no login (the public-route plan flagged below). New page + resolver live in Stage 06; tsc + next build + 517 unit tests green; tonone warden (SHIP) + proof reviewed. Next action: Adam review + merge `feat/public-ticket-page`. Earlier: **2026-06-16: overnight email harden (branch `chore/overnight-harden-2026-06-15`, NOT merged).** `rsvpConfirmedEmail`/`compTicketEmail` now render in Central (`America/Chicago`, were Eastern); em dashes removed from every subject/body (subjects to colon form); a hardening test suite (`tests/unit/email-hardening.test.ts`, `email-no-em-dash.test.ts`, `qr-route.test.ts`) locks no `data:` URIs, hosted https `/api/qr` images, `app.thenobadcompany.com` fallback, zero em dashes, and fail-closed `/api/qr`. FLAGGED (not built, awaiting approval): confirmation/comp fallback links still point at Clerk-gated `/m/.../confirmed` and `/check-in/verify` (WS1c public-route plan); the wrong-greeting-name report is data, not a template bug (WS1b). See `_context/_audit/EMAIL-SENDER-AUDIT-2026-06-16.md`. Earlier: (2026-06-13: branch `feat/stripe-checkout-hardening`, 10 commits, NOT merged — all 7 hardening phases done.) Merge after review, then: (1) `prisma db execute --file prisma/sql/stripe-event.sql` against prod Neon; (2) set `CRON_SECRET` in Vercel (capture-payments cron now scheduled in vercel.json); (3) Stripe test-mode dress rehearsal per `_context/_audit/STRIPE-TESTING.md`; (4) $1 live test. Earlier 2026-06-10 ticket-confirmation-email note is now subsumed. |
...
lib/ticketing/buyer-fee.ts                      ← buyer-pays-Stripe-fee gross-up + exact inverse split (pure, unit-tested; used by both access payment-intent routes + the pay-step line items)

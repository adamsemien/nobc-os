 ...
 *  mapping needed.
 *
 *  The only exception is the scored GROUP question `referrals`: the form writes
 *  its answers under `${id}.${subfield}` keys (e.g. referrals.referral1First),
 *  so it maps its stableKey to the list of subfield keys, and resolveAnswer
 *  joins the non-empty parts. Without this entry the group question would
...
export const MEMBERSHIP_ANSWER_KEY_BY_STABLE_KEY: Record<string, string | string[]> = {
  referrals: [
    'referrals.referral1First',
    'referrals.referral1Last',
    'referrals.referral2First',
    'referrals.referral2Last',

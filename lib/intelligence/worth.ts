 ...
 *  Worth (0-30) is derived from the 6 archetype scores, matching the
 *  convention used in the operator applications queue. */
...
export function worthTotal(scores: unknown): number {
  ...
  return (
    Math.round((g('Connector') + g('Curator')) / 20) +
    Math.round((g('Builder') + g('Maker')) / 20) +

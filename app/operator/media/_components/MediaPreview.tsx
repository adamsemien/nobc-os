...
export function MediaPreview({
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-black/85 md:flex-row">
      ...
      {/* Image / video area — flex-1 so it fills available space above the bottom sheet */}
      <div
        className="relative flex min-h-0 flex-1 items-center justify-center p-6 md:p-8"
        onClick={(e) => {
          e.stopPropagation();
          if (e.target === e.currentTarget) onClose();
        }}
      >
        {/* Close button — top-left on mobile (avoids top-right nav area), top-right on desktop */}
        <button
          aria-label="Close preview"
          className="absolute left-4 top-4 z-10 rounded-full p-2 text-white/80 transition-colors hover:bg-white/10 hover:text-white md:left-auto md:right-4"
          onClick={(e) => { e.stopPropagation(); onClose(); }}
        >
          <X className="h-6 w-6" />
        </button>

        {/* Prev / Next buttons — scoped inside image area, always safe */}
        {index > 0 && (
          <button
            aria-label="Previous"
      ...
      {/* Metadata panel:
          Mobile: bottom sheet — full width, max 45vh, scrollable so the image stays visible.
          Desktop (md+): right sidebar, 340px wide, full-height, scrollable. */}
      <aside

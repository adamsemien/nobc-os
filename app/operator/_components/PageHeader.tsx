          ...
          return (
            <li key={`${c.label}-${i}`} className="flex items-center gap-1.5">
              {c.href && !last ? (
                ...
                <span style={{ color: last ? 'var(--text-secondary)' : 'var(--text-muted)' }}>
                  {c.label}
                </span>
              )}
              {!last ? <span aria-hidden className="opacity-50">/</span> : null}
            </li>
          );
        })}
      </ol>
    ...
    <header className="mb-6">
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div className="min-w-0">
          <h1
            className="text-[26px] font-semibold tracking-tight text-text-primary font-[family-name:var(--font-dm-sans)]"
          >
            {title}
          </h1>
          {subtitle ? (

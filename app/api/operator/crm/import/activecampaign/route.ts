...
import { ALLOWED_AC_LISTS } from '@/lib/connectors/activecampaign/lists';
import {
...
/**
 * ActiveCampaign contact ingestion. Pulls contacts ONLY from the allowlisted relationship
 * lists (Network / Industry Partner / Sphere); the realtor book + the full-database dump
 * are firewalled off (see connectors/activecampaign/lists.ts). Maps each contact to a
 * NormalizedContact (subscriber role) and runs the same resolve → plan → persist pipeline
...
export async function POST() {
  ...
  let contacts;
  let pulledLists: string[] = [];
  try {
    const { contacts: acContacts, lists } = await client.fetchContactsForLists(ALLOWED_AC_LISTS);
    pulledLists = lists.map((l) => l.name);
  ...
  if (contacts.length === 0) {
    return NextResponse.json({ ok: true, fetched: 0, created: 0, attached: 0, deferred: 0, lists: pulledLists });
  }
